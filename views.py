...
def qualityreport(request):
    ...
    if request.method == 'POST':
        try:
            ...
            raw_data_query &= Q(l1_prod__end_time__range=(fromdate, todate))
            
            raw_data_values = raw_data.objects.filter(raw_data_query).values('l1_emp__employeeID', 'l2_emp__employeeID')
            ...
            for row in raw_data_values:
                ...
                if scope == 'DA1':
                    if l1_emp_id not in userlist:
                        userlist.add(l1_emp_id)
                elif scope == 'DA2':
                    
                    if l2_emp_id not in userlist:
                        userlist.add(l2_emp_id)
                elif scope == 'ALL' :
                    if l1_emp_id not in userlist:
                        userlist.add(l1_emp_id)
                    if l2_emp_id not in userlist:
...
def userwisequalityreport(userid,fromdate,todate,language_sl,filename,location,scope):
